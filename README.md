...
On the login screen, you'll see a dropdown with users: alice (ID 1), bob (ID 2), charlie (ID 3), admin (ID 999).
Select a user (e.g., alice) and click "Login as selected user".
You'll see the dashboard with that user's private data: User ID, Full Name, Email, Balance, and a Secret Note (hidden for normal users).
The app "authenticates" by storing the user ID in localStorage — this is the core insecurity.
To logout, click the "Logout" button — this removes the ID from localStorage and returns to login.
...
Log in as a normal user: Select "alice (user)" from the dropdown and click "Login".
Verify you're seeing Alice's data: User ID: 1, Full Name: Alice Wonderland, Email: alice@company.com, Balance: 1,250.00, Secret Note: Not visible to normal users.
Open DevTools: Press F12 (or Ctrl+Shift+I on Windows/Linux, Cmd+Option+I on Mac).
Go to the "Application" tab (in Chrome) or "Storage" tab (in Firefox).
Expand "Local Storage" → click on your site's domain (e.g., https://yourusername.github.io).
You'll see a key-value pair: userId with value "1".
Double-click the value "1" and change it to "2" (Bob's ID) — this is the IDOR exploit, as IDs are predictable and not checked.
Press Enter to save the change.
Refresh the page (F5 or Ctrl+R).
Now you're seeing Bob's private data without logging in as him: User ID: 2, Full Name: Bob Builder, Email: bob@company.com, Balance: 47,890.50.
Repeat: Change to "3" for Charlie's data.
Why this works: The app trusts client-side storage without validation — attacker guesses/changes the direct object reference (user ID).

Exploit 2: Vertical Privilege Escalation (Gain Admin Access)

Log in as a normal user (e.g., alice, ID 1).
Open DevTools → Application → Local Storage → change userId from "1" to "999" (admin's ID).
Press Enter to save.
Refresh the page.
Now you're "admin": User ID: 999, Full Name: System Administrator, Email: admin@company.com, Balance: ∞, and the Secret Note shows: "FLAG{you_just_bypassed_authorization_checks}".
This shows vertical escalation: Gaining higher privileges (admin secret visible) by tampering with the ID.
Note: In real apps, this could allow deleting users, viewing all data, etc. Here it's simulated with a flag.
